...
namespace AzureFunctions.Contrib.SignalR
{
    ...
    public class SignalRConfiguration : IExtensionConfigProvider
    {
        ...
        public void Initialize(ExtensionConfigContext context)
        {
            ...
            context.AddConverter<SignalRAttribute, SignalRConnectionBuilder>(attr => CreateConnectionInfoBuilder(attr));
            context.AddBindingRule<SignalRAttribute>()                
                .BindToCollector<SignalRMessage>(attr => new SignalRAsyncCollector(this, attr));

            context.AddBindingRule<SignalRAttribute>()
                .BindToInput<SignalRConnectionBuilder>(attr => CreateConnectionInfoBuilder(attr));
            //context.AddBindingRule<SignalRAttribute>()
            //    .BindToInput<SignalRMessage>(attr => CreateMessageFromAttribute(attr));
        }

        private SignalRConnectionBuilder CreateConnectionInfoBuilder(SignalRAttribute attr)
        {
            return new SignalRConnectionBuilder
            {
